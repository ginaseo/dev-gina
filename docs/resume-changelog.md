...
## 2026-09-22 — AI 활용 문구 수정

- `AI 활용 자동화` 표현을 `AI 활용 수정`으로 변경.
- 헤더 태그라인과 스킬 노트의 강조 문구를 동일하게 맞춤.
- 로컬/브라우저 캐시가 이전 `i18n.js`를 재사용하지 않도록 스크립트 URL에 버전 쿼리 추가.
- KRX Brief 문구에서 `코스피200`/`KOSPI200` 표현 제거.
- DevTicket 핵심 구현 문구를 주문·결제 이벤트 처리, 액션 로그, 부하 테스트 성과 중심으로 압축.

---

## 2026-07-23 — 포지션별 경로(`/backend`, `/ai`) 및 문구 조정

- `?v=` 쿼리 파라미터 대신 `/dev-gina/backend/`, `/dev-gina/ai/` 경로로도 접근 가능하도록 추가 (주소창이 쿼리 형태로 안 바뀌게 리다이렉트 대신 동일 페이지 복사본으로 처리).
- ai 포지션 태그라인을 "AI Agent 엔지니어 | LLM·에이전트 기반 자동화 파이프라인 설계"로 조정 (기존 "백엔드 개발자" 접두 제거).
- ai variant의 프로젝트/역량 노출 순서를 master와 동일하게 통일.
- backend/ai 태그라인 문구가 길어져 줄바꿈되던 문제 수정 (헤더 영역 폭 유연화).
